...
""".strip(),
    ...
    def test_old_aitgenne_input_media_failure_task_is_not_resumed(self):
        fields = {
            "视频错误信息": (
                "Aitgenne 首尾帧视频生成失败: {'error': {'code': 'InvalidParameter', "
                "'message': \"Field required: input.media & Input should be '1080P' or '720P': parameters.resolution\"}}"
            )
        }

        self.assertFalse(first_last.existing_video_task_matches_channel(
            fields,
            "Aitgenne",
    ...
    def test_render_video_uses_aitgenne_reference_video_for_happyhorse_model(self):
        ...
        self.assertEqual(submitter.call_args.kwargs["reference_urls"], ["https://x.test/ft_first.png", "https://x.test/ft_last.png"])
    ...
    def test_happyhorse_route_uses_exact_model_api_key(self):
        ...
        with patch.object(first_last, "TABLE_CONFIG", "tbl_config"), \
             patch.object(first_last, "safe_list_records", return_value=config_records):
            route = first_last.reference_video_route_for_model(
                "token",
                "Aitgenne",
                "Aitgenne / happyhorse-1.0-i2v",
                task_type="首尾帧视频",
    ...
    def test_happyhorse_route_does_not_reuse_other_aitgenne_key(self):
        ...
        with patch.object(first_last, "TABLE_CONFIG", "tbl_config"), \
            ...
            first_last.reference_video_route_for_model(
                "token",
                "Aitgenne",
                "Aitgenne / happyhorse-1.0-i2v",
                task_type="首尾帧视频",
