...
def render_first_frame(record_id: str, *, dry_run: bool = False) -> Dict[str, Any]:
    ...
    image_params = resolve_media_dimensions(fields, "首帧图", cfg, default_size=DEFAULT_OTU_IMAGE_SIZE)
    config_records = config_records_for_image_slot(fields, "首帧图", lambda: safe_list_records(token, TABLE_CONFIG))
    ...
    if existing_task_id:
        ...
        safe_update_record(token, TABLE_FIRST_LAST_VIDEO, record_id, filter_existing_fields(token, TABLE_FIRST_LAST_VIDEO, {
            ...
            "首帧图错误信息": f"已提交 {route.provider} 首帧图任务，正在轮询。task_id={submit_task_id}" if submit_task_id else "",
        }))
    if existing_task_id:
        image_result = run_image_generation(
            route,
    ...
    auto_review_summary = maybe_auto_advance_first_frame_review(token, record_id, fields, file_token=file_token)
...
def maybe_auto_advance_first_frame_review(token: str, record_id: str, fields: Dict[str, Any], *, file_token: str) -> Dict[str, Any]:
    ...
    if current_version(fields, "首帧图版本") != 1:
        return {"status": "manual_regeneration"}
    safe_update_record(token, TABLE_FIRST_LAST_VIDEO, record_id, filter_existing_fields(token, TABLE_FIRST_LAST_VIDEO, {
        "首帧审核状态": "通过",
        "错误信息": "",
    }))
    advance_summary = advance_first_review(record_id)
...
def render_last_frame(record_id: str, *, dry_run: bool = False) -> Dict[str, Any]:
    ...
    first_frame_token = latest_media_token(fields, "首帧图", "首帧图file_token")
    ...
    model_name = normalize_image_model_choice(cfg.get("model") or DEFAULT_OTU_IMAGE_MODEL)
    image_params = resolve_media_dimensions(fields, "尾帧图", cfg, default_size=DEFAULT_OTU_IMAGE_SIZE)
    ...
    if existing_task_id:
        ...
        image_result = run_image_generation(
            ...
            reference_image_paths=[ref["path"] for ref in product_refs if ref.get("path")],
    ...
    auto_review_summary = maybe_auto_advance_last_frame_review(token, record_id, fields, file_token=file_token)
    summary.update({"status": "success", "task_id": submit_task_id, "file_token": file_token, "output_path": out_path})
    summary["auto_review"] = auto_review_summary
    return summary
...
def maybe_auto_advance_last_frame_review(token: str, record_id: str, fields: Dict[str, Any], *, file_token: str) -> Dict[str, Any]:
    ...
    if not file_token:
        return {"status": "skipped", "reason": "missing_file_token"}
    if current_version(fields, "尾帧图版本") != 1:
        return {"status": "manual_regeneration"}
    safe_update_record(token, TABLE_FIRST_LAST_VIDEO, record_id, filter_existing_fields(token, TABLE_FIRST_LAST_VIDEO, {
        "尾帧审核状态": "通过",
        "错误信息": "",
    }))
    advance_summary = advance_last_review(record_id)
    return {"status": "auto_approved", "advance": advance_summary}
...
def submit_reference_video_task(
    ...
    *,
    ...
    if route.provider == "Aitgenne":
        ...
        return with_retry(_submit_aitgenne_once, max_attempts=4, label=f"submit first/last {route.provider} video {endpoint}")
    ...
    return with_retry(_submit_once, max_attempts=4, label=f"submit first/last {route.provider} video {endpoint}")
...
def poll_reference_video_task(route: ai_routing.AiRoute, task_id: str) -> Dict[str, Any]:
    ...
    while time.time() - start < 2400:
        ...
        if resp.status_code >= 400:
            raise RuntimeError(f"{route.provider} 首尾帧视频任务轮询失败: HTTP {resp.status_code}, body={str(last_body)[:1200]}")
        status = ai_routing.extract_video_status(last_body)
        if status in {"completed", "succeeded", "success", "done"}:
            return last_body
        if status in {"failed", "error", "cancelled", "canceled"}:
            raise RuntimeError(f"{route.provider} 首尾帧视频生成失败: {str(last_body)[:1200]}")
        time.sleep(8)
    raise TimeoutError(f"{route.provider} 首尾帧视频任务轮询超时: task_id={task_id}, last={str(last_body)[:1200]}")
...
def render_video(record_id: str, *, dry_run: bool = False) -> Dict[str, Any]:
    ...
    if existing_task_id:
        ...
        first_frame_token = latest_media_token(fields, "首帧图", "首帧图file_token")
        last_frame_token = latest_media_token(fields, "尾帧图", "尾帧图file_token")
        ...
        start_fields.update({
            ...
            "视频版本": version,
            "视频错误信息": (
                f"准备提交 AIHubMix Gemini/Veo 首尾帧视频任务... 视频画面尺寸={size}, native_resolution={native_resolution}"
                if channel == "AIHubMix"
                else f"准备提交 Aitgenne 首尾帧视频任务... {video_task_route_tag(channel, video_model['display'])}"
                if channel == "Aitgenne"
                else ""
            ),
        })
        ...
        if channel == "OTU":
            ...
            safe_update_record(token, TABLE_FIRST_LAST_VIDEO, record_id, filter_first_last_update_fields(token, {
                ...
                "视频版本": version,
                "视频生成原始响应JSON": compact_json({"submit": submit_body}, 10000),
                "视频错误信息": f"已提交 Aitgenne 首尾帧视频任务，正在轮询。{video_task_route_tag(channel, video_model['display'])} task_id={task_id}",
            }))
        else:
    ...
    if channel == "OTU":
        ...
        video_url = extract_video_url(result)
        if not video_url:
            raise RuntimeError(f"Aitgenne 首尾帧视频生成完成但未返回 video_url: {compact_json(result, 1200)}")
