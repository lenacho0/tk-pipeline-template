...
if [[ -n "$CA_BUNDLE" && -f "$CA_BUNDLE" ]]; then
  exec env TK_INSTANCE="$INSTANCE" TK_CONFIG_FILE="$CONFIG_FILE" TK_DISPATCHER_TABLE_KEY="$TABLE_KEY" REQUESTS_CA_BUNDLE="$CA_BUNDLE" SSL_CERT_FILE="$CA_BUNDLE" "$PYTHON_BIN" "$SCRIPT_DIR/tk_dispatcher.py" >> "$LOG_FILE" 2>&1
fi

exec env -u REQUESTS_CA_BUNDLE -u SSL_CERT_FILE TK_INSTANCE="$INSTANCE" TK_CONFIG_FILE="$CONFIG_FILE" TK_DISPATCHER_TABLE_KEY="$TABLE_KEY" "$PYTHON_BIN" "$SCRIPT_DIR/tk_dispatcher.py" >> "$LOG_FILE" 2>&1
