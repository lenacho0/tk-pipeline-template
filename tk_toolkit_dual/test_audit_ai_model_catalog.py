...
class AuditAiModelCatalogTests(unittest.TestCase):
    def test_report_marks_known_models_and_redacts_source_errors(self):
        api_results = {
            "OTU": {"models": ["gpt-image-2", "nano_banana_pro-4K", "veo_3_1-fast-fl"], "error": "Bearer sk-secret Invalid token"},
        ...
        self.assertNotIn("discard", report)
        self.assertNotIn("deprecated", report)
        self.assertNotIn("OTU / nano_banana_pro-4K", report)
        self.assertNotIn("AIHubMix / sora-2-pro", report)
        self.assertNotIn("Aitgenne / happyhorse-1.0-video-edit", report)
        self.assertIn("[REDACTED]", report)
        self.assertNotIn("sk-secret", report)
        self.assertNotIn("token=abc", report)
        self.assertNotIn("Invalid token", report)
