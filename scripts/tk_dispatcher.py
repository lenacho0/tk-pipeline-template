...
HEALTHCHECK_DONE_FILE = os.path.join(PROJECT_ROOT, f'.healthcheck_today.{INSTANCE}')
RUNNING_TASKS_FILE = os.path.join(PROJECT_ROOT, f'.running_tasks.{INSTANCE}.json')
RETRY_STATE_FILE = os.path.join(PROJECT_ROOT, f'.retry_state.{INSTANCE}.json')
METRICS_FILE = os.path.join(PROJECT_ROOT, f'.dispatcher_metrics.{INSTANCE}.json')
HEARTBEAT_FILE = os.path.join(PROJECT_ROOT, f'.dispatcher_heartbeat.{INSTANCE}.json')
DEAD_LETTER_FILE = os.path.join(PROJECT_ROOT, f'.dead_letter_tasks.{INSTANCE}.json')
CIRCUIT_BREAKER_FILE = os.path.join(PROJECT_ROOT, f'.circuit_breakers.{INSTANCE}.json')
STAGE_CFG = DISPATCHER_CFG.get('stages', {})
CIRCUIT_CFG = DISPATCHER_CFG.get('circuit_breaker', {})
TABLE_SCAN_STATE_FILE = os.path.join(PROJECT_ROOT, f'.table_scan_state.{INSTANCE}.json')
RECORD_STATE_CACHE_FILE = os.path.join(PROJECT_ROOT, f'.record_state_cache.{INSTANCE}.json')
...
RUNTIME_LOG_FILE = os.path.join(PROJECT_ROOT, f'dispatcher-runtime.{INSTANCE}.log')
PID_FILE = os.path.join(PROJECT_ROOT, f'dispatcher.{INSTANCE}.pid')
...
WATCH_LIST = [
    ...
    },
    {
        ...
        'failed_value': '下载失败',
        'error_field': '下载错误信息',
        'script': 'tk_pet_reference_analyze.py',
        'args': [],
        'timeout': 1800,
        'max_concurrency': 1,
        'max_retries': 1,
...
def get_table_records_cached(token, table_id, force=False):
    ...
    cache_file = os.path.join(PROJECT_ROOT, f'.table_cache_{table_id}.{INSTANCE}.json')
...
def write_pid_file():
    try:
        ...
        log.warning(f'保存 PID 文件失败 {PID_FILE}: {e}')


def main():
    log.info("🚀 TK 任务调度器启动（增强版 + 自动重试 + 运行统计）")
    log.info(f"   实例名: {INSTANCE}")
    log.info(f"   配置文件: {os.environ.get('TK_CONFIG_FILE', os.path.join(PROJECT_ROOT, 'config', 'config.json'))}")
